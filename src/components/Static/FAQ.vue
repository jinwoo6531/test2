      ...
      <v-tabs color="#000000de accent-4" centered fixed-tabs>
        ...
        <v-tab-item value="tab-1">
          ...
          <v-expansion-panels accordion flat>
            <v-expansion-panel
              style="border-bottom: 0.5px solid #BDBDBD;"
              v-for="(listItem, index) of visiblePages"
              :key="index"
            >
              <v-expansion-panel-header
                >Q{{ listItem.id }}.
                {{ listItem.subject }}</v-expansion-panel-header
              >
              <v-expansion-panel-content>
                {{ listItem.desc }}
              </v-expansion-panel-content>
        ...
        <v-tab-item value="tab-2">
          ...
          <v-expansion-panels accordion flat>
            <v-expansion-panel
              style="border-bottom: 0.5px solid #BDBDBD;"
              v-for="(listItem, index) of techVisiblePages"
              :key="index"
            >
              <v-expansion-panel-header
                >Q{{ listItem.id }}.
...
export default {
    ...
    items: [
      {
        ...
        desc:
          "좌석 11명, 입석 4명으로 최대 15명까지 탑승 가능하지만 오퍼레이터 및 안전을 위해 9명까지 권장합니다.",
      },
      {
        id: 2,
        subject: "최대 속도는 몇 km인가요?",
        desc:
          "최대 25km/h 까지 주행 가능하지만 국내 자율주행법의 영향을 받기 때문에 18km/h로 설정하여 주행합니다.",
      },
      {
        id: 3,
        subject: "히터 및 에어컨이 있나요?",
        desc:
          "둘 다 작동 가능하나 전기로 주행하다 보니 배터리가 더 빠르게 소모됩니다.",
      },
      {
        id: 4,
      ...
      },
      {
        ...
        desc:
          "네. 현재 국내 자율주행법에 의거하여 차량에 무조건 1명 이상의 안전요원이 있어야 합니다.",
      },
      {
        id: 6,
      ...
      },
      {
        ...
        desc:
          "3D 라이다 기준 100m 이며, 2D 라이다 기준으로 전후방 50m, 2D 라이다 측면 25m 감지 가능합니다.",
      },
    ],
    techItems: [
      {
  ...
  computed: {
    visiblePages() {
      ...
      ) // 0부터 10까지 자르기, 10부터 20까지 자르기
    },
    techVisiblePages() {
      return this.techItems.slice(
        (this.page - 1) * this.perPage,
        this.page * this.perPage
      ) // 0부터 10까지 자르기, 10부터 20까지 자르기
    },
