    ...
    <div id="userinfo">
        <v-container class="pa-0 ma-0 flex-wrap" fluid justify-center grid-list-md fill-height>
            <v-layout row wrap class="ma-0">
                            ...
                            <v-dialog v-model="namedialog" fullscreen hide-overlay transition="dialog-bottom-transition">
                                <v-card flat tile>
                                    <v-toolbar flat tile dense color="transparent">
                                        ...
                                        <v-spacer></v-spacer>
                                        <v-toolbar-items>
                                            <v-btn text class="pa-0" v-if="watch == false" disabled color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                                                    ">저장</v-btn>
                                            <v-btn text class="pa-0" v-else @click.native="showNameDialog" color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                                    ...
                                    <v-container class="pa-0 ma-0 flex-wrap" style="background: transparent">
                                        <v-row align="center" justify="center" class="pb-8 ma-0" width="100%">
                                            <v-card flat tile width="100%" class="pa-6" style="margin-top: 110px">
                                                ...
                                                <v-card-text class="pa-0" style="color: #eb5757">{{ rules }}
                                                </v-card-text>
                                            </v-card>
                                        </v-row>
                                    </v-container>
                            ...
                            <v-dialog v-model="emaildialog" fullscreen hide-overlay transition="dialog-bottom-transition">
                                <v-card flat tile>
                                    <v-toolbar flat tile dense color="transparent">
                                        ...
                                        <v-spacer></v-spacer>
                                        <v-toolbar-items>
                                            <v-btn text class="pa-0" v-if="watch1 == false" disabled color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                                                    ">저장</v-btn>
                                            <v-btn text class="pa-0" v-else @click.native="showEmailDialog" color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                            ...
                            <v-dialog v-model="genderdialog" fullscreen hide-overlay transition="dialog-bottom-transition">
                                <v-card flat tile>
                                    <v-toolbar flat tile dense color="transparent">
                                        ...
                                        <v-spacer></v-spacer>
                                        <v-toolbar-items>
                                            <v-btn text class="pa-0" v-if="watch3 == false" disabled color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                                                    ">저장</v-btn>
                                            <v-btn text class="pa-0" v-else @click.native="showGenderDialog" color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                            ...
                            <v-dialog v-model="birthdialog" fullscreen hide-overlay transition="dialog-bottom-transition">
                                <v-card flat tile>
                                    <v-toolbar flat tile dense color="transparent">
                                        ...
                                        <v-spacer></v-spacer>
                                        <v-toolbar-items>
                                            <v-btn text class="pa-0" v-if="watch2 == false" disabled color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                                                    ">저장</v-btn>
                                            <v-btn text class="pa-0" v-else @click.native="showBirthdialog" color="#2E3990" style="
                                                      font-family: Noto Sans KR;
                                                      font-style: normal;
                                                      font-weight: 500;
                                                      font-size: 18px;
                ...
                <v-flex xs12 sm12 md12 class="pa-0" style="
                            ...
                            <v-layout wrap class="pa-6">
                                ...
                                <v-flex xs12 sm12 md12 class="d-flex justify-left align-center">
                                    <v-card class="text-left" color="transparent" flat tile>
                                        <v-card-text class="pa-0 bye-content">
                                            이렇게 떠나시다니 너무 슬프네요. <br /> 왜 떠나는지 알려주신다면 <br /> 서비스 개선을 위해 더욱 힘쓸게요!
                                        </v-card-text>
                                    </v-card>
                                </v-flex>
                                <v-flex xs12 sm12 md12 class="d-flex justify-left align-top">
                                    <v-card class="text-left" color="transparent" flat tile width="100%">
                                        <v-card-text class="pa-0 pt-8 pb-2 bye-subcontent">
                                            본인 확인을 위해 <br /> 회원가입 시 사용한 휴대전화번호를 입력해주세요.
                                        </v-card-text>
                                ...
                                <v-flex xs12 sm12 md12 class="d-flex justify-center align-end">
                                    <v-footer padless absolute>
                                        <v-btn disabled block depressed tile height="50px" class="pa-0" v-if="
...
export default {
    ...
    computed: {
        ...
        // 전화번호를 '-'이 붙은 형태로 표시하기 위한 return 값
        getPhoneNumber() {
    ...
    watch: {
        ...
        displayName(newval) {
            ...
            // 이름의 길이가 한글 25자 이상, 영어 50자 이상을 넘거갈 경우 출력되는 경고
    ...
    methods: {
        ...
        // 변경 내용을 저장하지 않고 취소할 경우 기존 데이터 베이스에 있는 내용을 다시 가져온다.
        // 사용자의 uid로 조회
        nochangeDisplayName() {
