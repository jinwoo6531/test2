<template>
  <v-app style="position: relative;">
    <v-container fluid v-if="isLoading == true" class="auth-loading">
      ...
      <v-layout row wrap>
        <v-flex xs12 sm12 md12 class="d-flex flex-column justify-start align-center text-left">
          ...
          <v-card class="auth-sub-wrap pa-0" color="transparent" flat tile>
            <v-card-title class="pa-0 pb-2 auth-title">인증번호를 입력해주세요.</v-card-title>
            <v-card-text class="pa-0 auth-content">인증번호가 수신되지 않는 경우 스팸메시지 차단 기능을 확인하시기 바랍니다.</v-card-text>
          </v-card>
        </v-flex>
        <v-flex xs12 sm12 md12 class="d-flex flex-column justify-start align-center text-left">
...
export default {
  ...
  watch: {
    remainTime(val) {
      if (!val.indexOf("-") == -1) this.$router.go(-1);
    }
  },
