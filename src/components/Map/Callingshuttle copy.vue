    ...
    <v-layout row wrap class="ma-0">
      ...
      >
        <v-card flat tile color="transparent">
          ...
          <span class="arrive-time">약 {{ minutes }}분 후</span> 셔틀이 출발지에
          도착합니다.
  ...
  mounted() {
    ...
    this.socket.onmessage = ({ data }) => {
      ...
      if (
        ...
        this.webSocketData.how.site_id == this.site
      ) {
        this.socket.close();
        this.$router.replace({
          name: "AutoCancel",
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            if (this.site == 1) {
              ...
              this.map.setView([35.835153, 128.68562], 15);
              // } else if (this.site == 3) {
    ...
    getEta() {
      ...
      for (let [key, value] of Object.entries(eta)) {
        if (key == this.vehicle_id) {
          // 셔틀의 id와 ETA의 key 값이 같은 값의 value 가져오기
