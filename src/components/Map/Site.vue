            ...
            <v-row align="center" justify="center">
                <v-card flat tile class="cantServiceCard">
                    <v-card-text class="text-center pa-0">
                        ...
                        <p class="warningmsg">
                            <span class="warningmsg-font">호출이 자동 취소</span
                            >됩니다.
                        </p>
                        <v-btn
                            tile
                    ...
                    <v-flex class="pa-4 pt-0" xs12 sm12 md12>
                        <v-dialog v-model="dialog" fullscreen>
                            ...
                            <v-card>
                                <v-card-actions>
                                    <div
                                        style="width:100%;height: 95vh; background-color: #fff"
                                    >
                                        <div id="main">
                                ...
                                >
                                    ...
                                    >
                                        <v-card-text class="select-person-title"
                                            >{{ testtest }}
                                            <span class="select-person-title2"
                                                >탑승인원 선택</span
                                            >
                                        </v-card-text>
                        ...
                        >
                            <v-flex class="mb-3 spendTime-wrap">
                                <p
                                    class="ma-0"
                                    style="color: #fff; height: 1.875rem"
                                >
                                    <span
                                ...
                                <v-flex class="pa-0 flex-wrap" xs8 sm8 md8>
                                    <div class="d-flex flex-column">
                                        <v-card
                                            ...
                                            color="transparent"
                                            @click="
                                                daegu_timetable_popup = !daegu_timetable_popup
                                            "
                                            flat
                                        >
                                            <span
    ...
    computed: {
        ...
        totalPayment() {
            ...
            if (this.testtest === "종일 승차권") {
                num =
                    9000 * this.adultCount +
                    6000 * this.childCount +
                    0 * this.babyCount
            } else {
                num =
                    6000 * this.adultCount +
                    4000 * this.childCount +
                    0 * this.babyCount
            }
            num = parseInt(num, 10)
            return num.toLocaleString()
        ...
        start_options() {
            return this.stationList.filter(
                (station) => station.id != this.end.id
            )
        },
        end_options() {
            return this.stationList.filter(
                (station) => station.id != this.start.id
            )
        },
    },
    rideCount() {
    ...
    methods: {
        ...
        layerClickHandler(e) {
            ...
            if (this.start.id == markerVal.id || this.end.id == markerVal.id) {
                ...
                this.$utils.map.createDomEvent.addListener(
                    startSubmit,
                    "click",
                    () => {
                ...
                var endSubmit = this.$utils.map.getDomUtil(
                    `endBtn${markerVal.id}`
                )
            ...
            await axios
                ...
                .then(async (response) => {
                    if (response.status == 200) {
                        ...
                        this.stationList = this.stationList.sort(function(
                            a,
                            b
                        ) {
        ...
        async realTimeVehicle() {
            ...
            if (vehicle_arr.length) {
                vehicle_arr.forEach((vehicle) => {
                    this.vehicle[vehicle.id].setLatLng([
                        vehicle.lat,
                        vehicle.lon,
                    ]) // 위치 업데이트
                })
        ...
        babyIncrement() {
            if (this.totalCount < 14) {
                ...
                this.totalCount =
                    this.babyCount + this.adultCount + this.childCount
            ...
            axios
                ...
                .then((response) => {
                    ...
                    for (let i = 0; i < round_count; i++) {
                        this.timeTable.push(
                            round_result[i].time_start.substring(0, 5)
                        )
                    }
