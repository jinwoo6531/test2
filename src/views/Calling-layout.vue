...
export default {
  ...
  watch: {
    socket() {
      ...
      // Web Socket Error 또는 Web Socket 서버 접속이 끊길 경우에는 연결을 끊고 다시 연결한다.
  ...
  methods: {
    ...
    callCancelModal() {
      // WebSocket Cancel
      this.cancelMessage();
      this.disconnect(); // Web socket 연결 끊기
    ...
    onMessageWebSocket() {
      this.socket.onmessage = ({ data }) => {
        ...
        console.log(this.webSocketData.what);
        if (
